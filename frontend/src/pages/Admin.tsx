    ...
    return (
      <div className="h-full flex items-center justify-center px-4 cursor-none">
        <motion.div 
            ...
            className="glass-panel max-w-sm w-full rounded-3xl p-6 space-y-6 text-center relative overflow-hidden bg-black/60 border border-white/5"
        >
            ...
            <div className="space-y-3">
                <h3 className="text-2xl font-black italic tracking-tighter uppercase glow-text">Command Access</h3>
                <p className="text-white/80 text-[9px] font-bold tracking-[0.3em] uppercase italic">Authorized Personnel Node</p>
            </div>
            ...
            <form onSubmit={handleLogin} className="space-y-6">
                <input 
                    ...
                    className="w-full bg-black/40 border border-white/40 rounded-2xl py-4 px-6 text-center text-2xl tracking-[0.6em] font-black focus:outline-none focus:border-primary/60 transition-all font-mono text-white placeholder:text-white"
                    value={pin}
    ...
    <div className="h-full flex flex-col gap-3 max-w-7xl mx-auto cursor-none">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-5">
            ...
            <div>
                <h2 className="text-xl font-black italic tracking-tighter uppercase text-white glow-text leading-none glitch-hover">Admin Control Console</h2>
      ...
      <div className="flex-1 min-h-0 grid grid-cols-1 xl:grid-cols-4 gap-3">
          ...
          <div className="xl:col-span-1 grid grid-cols-2 xl:grid-cols-1 gap-3 min-h-0">
              {[
                  ...
                  <motion.div 
                    ...
                    key={i} 
                    className="glass-panel rounded-3xl p-4 border border-white/5 bg-black/40 flex flex-col justify-center relative overflow-hidden group"
                  >
          ...
          <div className="xl:col-span-3">
              ...
              <div className="glass-panel rounded-[2rem] overflow-hidden border border-white/5 bg-black/60 flex flex-col flex-1 min-h-0">
                  <div className="p-4 border-b border-white/5 flex items-center justify-between shrink-0 bg-white/[0.02]">
                      ...
                      <AnimatePresence>
                          {messages.length > 0 ? messages.map((msg, i) => (
                              <motion.div 
                                  ...
                                  transition={{ delay: i * 0.05 }}
                                  key={msg.id} 
                                  className="p-4 hover:bg-white/[0.03] cursor-none transition-all group relative border-l-2 border-transparent hover:border-primary"
                              >
                                  <div className="flex justify-between items-start mb-2">
                                      <div className="flex items-center gap-5">
                                          <div className="h-10 w-10 rounded-2xl bg-black/60 border border-white/10 flex items-center justify-center text-xs font-black uppercase tracking-tighter group-hover:border-primary/40 group-hover:text-primary transition-all">
                                              {(msg.name || '??').substring(0, 2)}
                                          </div>
                                          <div>
                                              <p className="text-sm font-black italic tracking-tight text-white/90 leading-tight group-hover:text-white transition-colors uppercase">{msg.name}</p>
                                              <p className="text-[9px] text-white/10 font-mono tracking-[0.2em] uppercase">{msg.email}</p>
                                          </div>
                                      </div>
